...
class Mimii_due(Dataset):
    ...
    def __init__(self, target_dir, section_name, dir_name, mode='development', extraction_type='amplitude', n_mfcc=40):
    ...
    def __init_file_list_generator(self):
        ...
        # development
        if self.mode:
            query = os.path.abspath("{target_dir}/{dir_name}/{section_name}_*_normal_*.wav".format(target_dir=self.target_dir,
            ...
            query = os.path.abspath("{target_dir}/{dir_name}/{section_name}_*_anomaly_*.wav".format(target_dir=self.target_dir,
                                                                                                    dir_name=self.dir_name,
                                                                                                    section_name=self.section_name,
                                                                                                    ))
            ...
            self.file_list = np.concatenate(
                (normal_files, anomaly_files), axis=0)
            self.labels = np.concatenate(
                (normal_labels, anomaly_labels), axis=0)

        # evaluation
        else:
            query = os.path.abspath("{target_dir}/{dir_name}/{section_name}_*.wav".format(target_dir=self.target_dir,
                                                                                          dir_name=self.dir_name,
                                                                                          section_name=self.section_name,
                                                                                          ))
            self.file_list.append(sorted(glob.glob(query)))
    ...
    @staticmethod
    def __feature_extraction_from_file(file, extraction_type, n_mfcc):
    ...
    def __file_list_to_data(self):
        ''' iteration each file to vector array()'''
        if self.extraction_type == 'melspectrogram':

            for idx in tqdm(range(len(self.file_list))):

                vectors = Mimii_due.__feature_extraction_from_file(
                    self.file_list[idx], self.extraction_type, self.n_mfcc)
                vectors = vectors[:: 1, :]
                if idx == 0:
                    self.data = np.zeros(
                        (len(self.file_list) * vectors.shape[0], 1, self.n_mfcc, vectors.shape[-1]), float)
                self.data[vectors.shape[0] * idx: vectors.shape[0]
                          * (idx + 1), :] = vectors
...
class Toyadmos(Dataset):
    ...
    def __init__(self, target_dir, dir_name_normal, dir_name_anomaly, extraction_type='amplitude', n_mfcc=40):
    ...
    def __init_file_list_generator(self):
        query_norm = os.path.abspath("{target_dir}/{dir_name_normal}/*.mp4".format(target_dir=self.target_dir,
                                                                                   dir_name_normal=self.dir_name_normal))
        normal_files = sorted(glob.glob(query_norm))
        normal_labels = np.zeros(len(normal_files))

        query_anomaly = os.path.abspath("{target_dir}/{dir_name_anomaly}/*.mp4".format(target_dir=self.target_dir,
                                                                                       dir_name_anomaly=self.dir_name_anomaly))
    ...
    def __file_list_to_data(self):
        ...
        if self.extraction_type == 'melspectrogram':
            ...
            for idx in tqdm(range(len(self.file_list))):
                ...
                vectors = Toyadmos.__feature_extraction_from_file(
                    self.file_list[idx], self.extraction_type, self.n_mfcc)
                vectors = vectors[:: 1, :]
                if idx == 0:
                    self.data = np.zeros(
