...
class ElEngine(Dataset):
    ...
    def _init_file_list_generator(self):
        ...
        normal_files = sorted(list(query_norm.glob('*.wav')))
        ...
        if self.dir_name_anomaly:
            ...
            anomaly_files = sorted(list(query_anomaly.glob('*.wav')))
    ...
    def _feature_extraction_from_file(file, extraction_type, sr, n_mfcc, n_mel):
        ...
        if extraction_type == "aggregate_features":
            ...
            features = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=n_mfcc, hop_length=2400)
    ...
    def _file_list_to_data(self):
        ...
        """
        if self.extraction_type == "melspectrogram" or self.extraction_type == 'spectrogram' or self.extraction_type == 'mfccs' or self.extraction_type == 'mfccs_delta':

            for idx in tqdm(range(len(self.file_list))):

                vectors = ElEngine._feature_extraction_from_file(
