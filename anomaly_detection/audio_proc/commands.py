...
def record_single(dir_name="sound_rec/train_unlabled", channels=1, rate=16000, frames_per_buffer=1024, duration=11, input_device_index=None):
    record_sound(
        dir_name,
        channels,
        rate,
...
def record_two(dir_with_wav="sound_rec/record_buffer", target_dir="sound_rec/flac", channels=1, rate=16000, frames_per_buffer=1024, duration=11, input_device_index_1=0, input_device_index_2=1):
    def thr():
        while True:
    ...
    schedule.every(1).minutes.do(
        to_flack, dir_with_wav, target_dir
    )
    Thread(target=thr).start()
    # recording first mic (device [0])
    Thread(target=record_sound, args=(dir_with_wav, channels, rate, frames_per_buffer, duration, input_device_index_1)).start()
    time.sleep(1)
    # recording second mic (device [1])
    Thread(target=record_sound, args=(dir_with_wav, channels, rate, frames_per_buffer, duration, input_device_index_2)).start()
